...
import MarkerWithLabelCreator, {
  markerDefaultPropTypes,
  markerControlledPropTypes,
  markerEventPropTypes,
} from './MarkerWithLabelCreator';

export default class MarkerWithLabel extends Component {
  static propTypes = {
  ...
  componentWillMount() {
    ...
    const marker = MarkerWithLabelCreator._createMarker(this.props);
  ...
  render() {
    if (this.state.marker) {
      return (
        <MarkerWithLabelCreator marker={this.state.marker} {...this.props}>
          {this.props.children}
        </MarkerWithLabelCreator>
      );
    } else {
      return (<noscript />);
    }
